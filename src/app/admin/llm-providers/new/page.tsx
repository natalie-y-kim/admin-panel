...
type NewLlmProvidersPageProps = {
  searchParams: Promise<{
    error?: string;
  }>;
};

export default async function NewLlmProvidersPage({
  searchParams,
}: NewLlmProvidersPageProps) {
  await requireSuperadmin();
  const params = await searchParams;

  return (
    <section className="mx-auto w-full max-w-2xl rounded-xl bg-white p-6 shadow-sm">
      <h1 className="text-2xl font-semibold text-slate-900">New LLM Provider</h1>
      <p className="mt-2 text-sm text-slate-600">Create a new LLM provider.</p>
      ...
      <form action={createLlmProviderAction} className="mt-6 space-y-5">
        <div>
          <label
            htmlFor="name"
