...
type EditCaptionExamplesPageProps = {
...
export default async function EditCaptionExamplesPage({
  params,
  searchParams,
}: EditCaptionExamplesPageProps) {
  await requireSuperadmin();
  const { id } = await params;
  const query = await searchParams;
  const supabase = await createClient();

  const [{ data: example, error }, { data: images }] = await Promise.all([
    supabase
      .from("caption_examples")
      .select("id, image_description, caption, explanation, priority, image_id")
      .eq("id", id)
      .maybeSingle(),
    supabase.from("images").select("id, url").order("created_datetime_utc", { ascending: false }),
  ]);

  if (error) {
    return (
      <section className="mx-auto w-full max-w-2xl rounded-xl bg-white p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-slate-900">Edit Caption Example</h1>
        <p className="mt-4 rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          Unable to load this caption example right now.
        </p>
        <Link
  ...
  return (
    <section className="mx-auto w-full max-w-2xl rounded-xl bg-white p-6 shadow-sm">
      ...
      <p className="mt-2 text-sm text-slate-600">
        Update caption example fields and save.
      </p>
      ...
      <form action={updateCaptionExampleAction} className="mt-6 space-y-5">
